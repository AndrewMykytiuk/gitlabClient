...
class MergeRequestStorageService: MergeRequestStorageServiceType {
    ...
    func createEntity(with mergeRequest: MergeRequest) -> MergeRequestEntity {
        guard let entity = NSEntityDescription.entity(forEntityName: entityName(), in: storage.childContext) else { fatalError(GitLabError.Storage.EntityCreation.failedMergeRequest.rawValue) }
