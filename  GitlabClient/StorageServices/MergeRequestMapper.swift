...
class MergeRequestMapper {
    ...
    func setupMergeRequest(with mergeRequestEntity: MergeRequestEntity) -> MergeRequest {
        ...
        guard let assigneeEntity = mergeRequestEntity.assignee, let authorEntity = mergeRequestEntity.author else { fatalError(GitLabError.Storage.CoreDataEntityMapper.failedMergeRequestMap.rawValue) }
